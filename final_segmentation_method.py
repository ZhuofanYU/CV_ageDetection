...
def calculate_average_size(filename, plot_steps=False, n_image=200, m_image=400, n_counting=100, m_counting=100):
    ...
    def plot(img, n, m, title=''):
        ...
        if n < img.shape[0] and m < img.shape[1]:
            #        print('ok')
            plt.imshow(img[img.shape[0] // 2 - n: img.shape[0] //
                           2 + n, img.shape[1] // 2 - m: img.shape[1] // 2 + m])
            plt.show()
        else:
            plt.imshow(img)
            plt.show()
    ...
    original = original[img.shape[0] // 2 - n: img.shape[0] //
                        2 + n, img.shape[1] // 2 - m: img.shape[1] // 2 + m]
    ...
    total_components, markers, stats, centroids = cv2.connectedComponentsWithStats(
        G, connectivity=8)
    markers[G == 0] = 0
    ...
    if plot_steps:
        plot(mark_boundaries(original, watershed_markers),
             n, m, 'Watershed Boundaries 500*800')
        plot(mark_boundaries(original, watershed_markers),
             100, 100, 'Watershed Boundaries 100*100')
    ...
    for i in range(1, total_components):
        ...
        background_samples = original[np.logical_and(
            r == -1, watershed_markers == i)]
        ...
        res[np.logical_and(original <= max_percentile,
                           watershed_markers == i)] = 0
    ...
    hole_total_components, hole_markers, hole_stats, hole_centroids = cv2.connectedComponentsWithStats(
        255 - G_post, connectivity=4)
    ...
    if plot_steps:
        ...
        plot(original_median, n=50, m=50,
             title='Comparison with the original image 100*100')

        plot(G_post, 200, 400, title='Final segmentation result for our method 400*800')
        plot(original_median, n=200, m=400,
             title='Comparison with the original image 400*800')
    ...
    n = n_counting
    m = m_counting
    G_post = G_post[G_post.shape[0] // 2 - n: G_post.shape[0] //
                    2 + n, G_post.shape[1] // 2 - m: G_post.shape[1] // 2 + m]
    ...
    num_cells, markers, stats, centroids = cv2.connectedComponentsWithStats(
        G_post, connectivity=4)
...
if __name__ == "__main__":
    avg_size = calculate_average_size(filename, plot_steps=True)
